<?php
	...
	if ($mmhclass->info->config['gallery_viewing'] == 0 && $mmhclass->info->is_admin == false || $mmhclass->info->config['useronly_uploading'] == true) {
		...
		if ($mmhclass->db->total_rows($sql) < 1) {
			...
			while ($row = $mmhclass->db->fetch_array($sql)) {
				...
				$mmhclass->templ->templ_vars[] = array(
					...
					"FILENAME_THUMB" => $imageinfo['filename_thumb'],
