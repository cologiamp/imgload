<?php
	...
	// LTE: 1251314996 - Wednesday, August 26, 2009, 03:29:56 PM EDT -0400
	// ======================================== /
	...
	class mmhclass_mysql_driver
	{
		...
		function connect($host = MYSQL_DEFAULT_CONNECT_HOST, $username, $password, $database, $port = MYSQL_DEFAULT_CONNECT_PORT, $boolerror = false)
		{
			if (USE_MYSQL_LIBRARY == false) {
				$this->mmhclass->templ->fatal_error("Sorry, but ImgPizza will not work without MySQL loaded as a PHP extension.");
			} else {
		...
		function error($query = "No Query Executed", $custom_error = NULL)
		{
			...
			if ($error_fileh = fopen(sprintf("%ssource/errorlog/mysql/%s.log", $mmhclass->info->root_path, date("m-d-Y")), "ab")) {
