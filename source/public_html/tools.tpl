...
<span id="uploader_iframe">
    <form action="<# BASE_URL #>upload.php" target="_blank" method="post" enctype="multipart/form-data">
        <if="$mmhclass->templ->templ_globals['upload_type'] == 'url'">
            ...
            <input type="submit" value="Upload"> - <a href="javascript:void(0);" onclick="$('#iframe_uploader').html(get_ajax_content('<# BASE_URL #>index.php?module=iframeupload'));">Normal Upload</a>
        <else> 
            <input type="file" name="userfile[]" size="27" /> 
            <input type="hidden" name="upload_type" value="standard" />
            <input type="submit" value="Upload"> - <a href="javascript:void(0);" onclick="$('#iframe_uploader').html(get_ajax_content('<# BASE_URL #>index.php?module=iframeupload&url=1'));">URL Upload</a>
        </endif>
    </form>
</span>
