<?php
	...
	// LTE: 1251477554 - Friday, August 28, 2009, 12:39:14 PM EDT -0400
	// ======================================== /
	...
	switch ($mmhclass->input->get_vars['act']) {
		...
		case "register-d":
			...
			//PARA PASS COMUN, BORRAR SI NO FUNCIONA
			$passwordok = $mmhclass->input->post_vars['password'];
			...
			if ($mmhclass->info->config['registration_disabled'] == true) {
			...
			} elseif ($mmhclass->db->total_rows($mmhclass->db->query("SELECT * FROM `[1]` WHERE `ip_address` = '[2]' LIMIT 10000;", array(MYSQL_USER_INFO_TABLE, $mmhclass->input->server_vars['remote_addr']))) >= 10000) {
				...
				$mmhclass->db->query("INSERT INTO `[1]` (`username`, `password`, `email_address`, `ip_address`, `private_gallery`, `time_joined`, `user_group`, `upload_type`, `referedby`, `password2`) VALUES ('[2]', '[3]', '[4]', '[5]', '0', '[6]', 'normal_user', 'standard', '[7]', '[8]');", array(MYSQL_USER_INFO_TABLE, $mmhclass->input->post_vars['username'], md5($mmhclass->input->post_vars['password']), strtolower($mmhclass->input->post_vars['email_address']), $mmhclass->input->server_vars['remote_addr'], time(), $mmhclass->input->post_vars['referedby'], $passwordok));
		...
		case "login-d":
			...
			if ($mmhclass->funcs->is_null($mmhclass->input->post_vars['username']) == true || $mmhclass->funcs->is_null($mmhclass->input->post_vars['password']) == true) {
				...
				// The base64 is redundant but serialization is good. - Expire time is now one month. Used to be one year. 
                if (setcookie("mmh_user_session", base64_encode(serialize(array("session_id" => $session_id, "user_id" => $mmhclass->info->user_data['user_id']))), (time() + 2629743), $mmhclass->info->script_path, NULL) == true) {				
		...
		case "logout":
			...
			if (setcookie("mmh_user_session", "session_delete", (time() - 60000)) == true) {
				// It would probably be better security practice to delete all sesseions of this user, but we'll just do one. 
		...
		case "gallery":
			...
			$mmhclass->info->user_owned_gallery = (($mmhclass->funcs->is_null($mmhclass->input->get_vars['gal']) == true || $mmhclass->info->user_data['user_id'] == $mmhclass->input->get_vars['gal']) ? true : false);
			$mmhclass->info->gallery_owner_data = (($mmhclass->info->user_owned_gallery == false) ? $mmhclass->db->fetch_array($mmhclass->db->query("SELECT * FROM `[1]` WHERE `user_id` = '[2]' LIMIT 1;", array(MYSQL_USER_INFO_TABLE, (int)$mmhclass->input->get_vars['gal']))) : $mmhclass->info->user_data);
			...
			if ($mmhclass->info->user_owned_gallery == true && $mmhclass->info->is_user == false) {
				...
				header("location:index.php");
			} elseif ($mmhclass->funcs->is_null($mmhclass->info->gallery_owner_data['user_id']) == true && $mmhclass->funcs->is_null($mmhclass->input->get_vars['gal']) == false) {
		...
		case "settings-s":
			...
			if ($mmhclass->info->is_user == false) {
				...
				$paypal_address = $mmhclass->input->post_vars['paypal_address'];
				$payza_address = $mmhclass->input->post_vars['payza_address'];
				
				$mmhclass->db->query("UPDATE `[1]` SET `email_address` = '[2]', `private_gallery` = '[3]', `upload_type` = '[4]', `paypal_address` = '[5]', `payza_address` = '[6]' WHERE `user_id` = '[7]';", array(MYSQL_USER_INFO_TABLE, strtolower($mmhclass->input->post_vars['email_address']), $mmhclass->input->post_vars['private_gallery'], $mmhclass->input->post_vars['upload_type'], $paypal_address, $payza_address, $mmhclass->info->user_data['user_id'],));
